...
class TestUserCreationAndAuth:
    ...
    async def test_create_user(
        self, client: AsyncClient, db_session: AsyncSession, test_user_credentials: dict
    ):
        """
    ...
    async def test_create_user_already_exists(
        self, client: AsyncClient, created_test_user: dict, test_user_credentials: dict
    ):
        """
...
class TestAdminActions:
    ...
    async def test_admin_can_read_all_users(
        self, authenticated_admin_client: AsyncClient, created_test_user: dict
    ):
        """
    ...
    async def test_normal_user_cannot_read_all_users(
        self, authenticated_user_client: AsyncClient
    ):
        """
    ...
    async def test_admin_can_change_user_role(
        ...
        response = await authenticated_admin_client.patch(
            f"/users/admin/users/{username_to_change}/role", json={"role": "admin"}
        )
    ...
    async def test_admin_cannot_demote_last_admin(
        ...
        """
        response = await authenticated_admin_client.patch(
            "/users/admin/users/adminuser/role", json={"role": "user"}
        )

        assert response.status_code == 409
        assert (
            "Cannot demote user 'adminuser': this is the last administrator."
    ...
    async def test_admin_cannot_delete_last_admin(
        self, authenticated_admin_client: AsyncClient, db_session: AsyncSession
    ):
        """
        ...
        assert response.status_code == 409
        assert (
            "Cannot delete user 'adminuser': this is the last administrator."
            in response.json()["detail"]
        )
