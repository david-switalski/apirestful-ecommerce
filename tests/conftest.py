...
TEST_DATABASE_URL = (
    f"postgresql+asyncpg://{db_user}:{db_password}@{db_host}:{db_port}/test_{db_name}"
)
MAINTENANCE_URL = (
...
)  # Use DB 1 for testing to isolate from dev DB 0
...
@pytest.fixture(scope="function")
async def client(
    db_session: AsyncSession, test_redis_client: redis.Redis
...
async def authenticated_admin_client(
    ...
    admin_user = UserModel(
        username=admin_username, hashed_password=hashed_password, role="admin"
    )
