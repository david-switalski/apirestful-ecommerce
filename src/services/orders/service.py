...
class OrderService:
    ...
    async def create_order(self, order_data: OrderCreate, current_user: UserModel) -> ReadOrder:
    ...
    async def get_order_by_id_for_user(self, order_id: int, user_id: int) -> ReadOrder | None:
        order_model = await self.order_repo.get_by_id_and_user(order_id, user_id)
        if order_model:
            return ReadOrder.model_validate(order_model)
        return None

    async def get_all_orders_for_user(self, user_id: int, limit: int, offset: int) -> list[ReadOrder]:
