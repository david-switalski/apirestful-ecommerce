...
class ProductRepository:
    ...
    async def get_by_name(self, product_name: str) -> ProductModel | None:
        result = await self.db.execute(select(ProductModel).where(ProductModel.name == product_name))
        return result.scalars().first()

    async def get_all(self, limit: int, offset: int) -> list[ProductModel]:
        results = await self.db.execute(select(ProductModel).limit(limit).offset(offset))
    ...
    async def get_by_id_for_delete_validation(self, product_id: int) -> ProductModel | None:
        stmt = (
            select(ProductModel)
    ...
    async def get_many_by_ids_with_lock(self, product_ids: list[int]) -> list[ProductModel]:
        stmt = (
            select(ProductModel)
